...
void VideoFaceDetector::FaceTracker()
{
	...
	while (1) // nonstop searching or tracking
	{
		...
		else // get ROI in frame and re-detect
		{
			...
			//cv::Mat imgt = imread("imgROI.jpg");
			//FR_Rect RGBRectu[50]; // 50 faces at most. May fail if num of face in frame > 50
			//int iFaceNumRGBt = FD_Detect(FD_handle, imgt.data, imgt.cols, imgt.rows, RGBRectu);
			//std::cout << "# of face(s) found in saved ROI : " << iFaceNumRGBt << std::endl;
			//int iFaceNumROI = FD_Detect(FD_handle, imgROI.data, imgROI.cols, imgROI.rows, ROIRect);
...
void VideoFaceDetector::CreateUserDB()
{
	...
	if ((dir = opendir("FaceDataBase\\")) != NULL) { // set face data base dir here.
		...
		while ((ent = readdir(dir)) != NULL) {
			...
			{
				//std::cout << "Before " << clock() << std::endl;
				ExtractAndSearch(rect[0], img, UserDatabase, FaceDetectRecogResult, FF_handle, 0);
...
void VideoFaceDetector::LocalVerifier()
{
	...
	while (1)
	{
		std::this_thread::sleep_for(std::chrono::milliseconds(1000 / 100)); // 100 FPS
